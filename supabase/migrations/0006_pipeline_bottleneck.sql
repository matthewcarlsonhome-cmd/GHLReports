-- Pipeline bottleneck: the single stage holding the most idle dollars, as
-- snapshot columns so the portfolio can show it without opening the details
-- blob. Label carries the pipeline name only on multi-pipeline accounts.
alter table public.snapshots add column bottleneck_stage text;
alter table public.snapshots add column bottleneck_value_usd numeric;

-- v_portfolio unchanged except the two bottleneck columns appended at the
-- end (create or replace view only allows adding columns at the end).
