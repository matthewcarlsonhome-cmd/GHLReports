-- Client staff vs SSP staff per account (docs/ACCOUNT-USAGE.md), and the
-- v_portfolio security fix.
--
-- 1. snapshots.client_users / ssp_users: GHL marks every user roles.type
--    "account" (the client's own team) or "agency" (SSP). An account with no
--    client users is ads delivery only: nobody at the client can log in, so
--    it stays out of the AM digest and is hidden by default in the
--    portfolio (a toggle shows it). NULL = not collected yet / users fetch
--    failed, which never hides anything.
-- 2. Backfill: snapshots whose users fetch succeeded and returned nobody at
--    all get 0/0 now (no role data needed to know that).
-- 3. v_portfolio is recreated WITH (security_invoker = on). Migration 0010
--    replaced the view without it, so since then the view ran with its
--    owner's rights and skipped the is_staff() row policies underneath.
--    Columns are only appended, as CREATE OR REPLACE VIEW requires.
alter table public.snapshots
  add column if not exists client_users int,
  add column if not exists ssp_users int;

update public.snapshots s
   set client_users = 0, ssp_users = 0
 where s.client_users is null
   and s.details->'users' = '{}'::jsonb
   and s.coverage->'sources'->'users'->>'status' = 'complete';
